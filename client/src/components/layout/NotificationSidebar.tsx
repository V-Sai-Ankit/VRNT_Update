...
export function NotificationSidebar() {
  ...
  // Return the minimized bell button if it is minimized
  if (isMinimized) {
    return (
      <motion.button
        ...
        className="fixed right-0 top-1/2 -translate-y-1/2 z-[60] bg-primary text-primary-foreground p-2 md:p-3 rounded-l-2xl shadow-2xl border-y border-l border-white/20 hover:pr-5 transition-all group"
      >
        <div className="flex flex-col items-center gap-2">
          <Bell className="animate-pulse" size={18} />
          <span className="[writing-mode:vertical-lr] text-[9px] md:text-[10px] font-bold uppercase tracking-widest py-1 md:py-2">Notifications</span>
        </div>
      </motion.button>
    );
  }
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed right-0 top-0 bottom-0 md:top-[180px] md:bottom-8 w-full sm:w-[350px] z-[70] p-0 sm:px-4 pointer-events-none"
      >
        <div className="bg-white md:bg-white/95 backdrop-blur-md h-full sm:rounded-2xl border-l md:border border-primary/20 shadow-[0_20px_50px_rgba(0,0,0,0.15)] flex flex-col pointer-events-auto overflow-hidden">
          {/* Header */}
          ...
          <div className="p-4 border-t border-border bg-muted/30">
            <Link href="/news">
              <a className="block w-full text-center py-3 md:py-2 bg-secondary text-secondary-foreground rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-secondary/90 transition-colors">
                See All Updates
              </a>
            </Link>
          </div>
